...
| V9+ | V9 is ROBUST to drug- and patient-subsetting (B3d) | Drug jackknife all leave-one-out >0 (min +0.033); bootstrap 95% CI over drugs [+0.008,+0.074] excludes 0; internal patient split-half both halves >0 (+0.053/+0.027). Not driven by any single drug or patient subset. EXPLORATORY (not a pre-registered test): best-transferring drugs are AML-relevant agents — venetoclax, cytarabine, sorafenib, MEK inhibitors selumetinib/trametinib (connects to V5 NRAS→MEK) — mechanistically coherent, needs its own pre-registered confirmation. | B3d_metrics.json |
