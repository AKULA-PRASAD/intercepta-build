...
> **ERRATA — DRIFT FINDINGS 4, 5, 6 RESOLUTION (2026-05-11):**
>
> The 2026-05-11 corpus-read audit by CSO surfaced three internal inconsistencies in L2.1 as originally written. This errata block resolves all three with BINDING clarifications. Implementation code below is unchanged; the errata is the operative reading guide.
>
> **Finding 5 — §1.2 ↔ §6.2 Dimensionality Conversion Redundancy (BINDING RESOLUTION):**
>
> §1.2 defines `SubstrateInterface.project_to_canonical(embedding) → (n_cells, 512)` as the substrate-resident projection mechanism. §6.2 example code shows the L7 head instantiating `torch.nn.Linear(substrate.output_dim, 512)` directly. These are two parallel mechanisms for the same conversion.
>
> **BINDING RESOLUTION:** The §1.2 substrate-resident `project_to_canonical` method is the canonical and required mechanism. The §6.2 example `torch.nn.Linear(substrate.output_dim, 512)` is INCORRECT as written — it bypasses the substrate's own projection logic (which includes zero-padding for NATIVE_DIM < 512, learned projection for NATIVE_DIM > 512, and the `fit_canonical_projector` lifecycle). L2.2 (when written) must consume substrates as: `cell_emb_canonical = substrate.project_to_canonical(substrate.encode(adata))`, NOT via L7-internal `torch.nn.Linear`. This binding is enforced for L2.2 specification.
>
> The §6.2 example code remains in this document for narrative continuity but is functionally superseded by this errata. L2.2 spec will use the correct pattern.
>
> **Finding 6 — scTOP NATIVE_DIM Lifecycle (BINDING CLARIFICATION):**
>
> §1.2 declares `NATIVE_DIM: int = -1` as a class-level default for `SubstrateInterface`. §3.3 (scTOP implementation) shows `NATIVE_DIM = -1 # determined at fit time by reference cell type count` and `self.NATIVE_DIM = reference_basis.shape[1]` set inside `fit()` / `load_pretrained()`. §13.1 quick-reference table shows scTOP as `Native Dim: n_celltypes (variable)`. These three statements are consistent but the lifecycle ordering is implicit and easy to misread.
>
> **BINDING CLARIFICATION:** scTOP's `NATIVE_DIM` is `-1` (sentinel) at instantiation and is set to `reference_basis.shape[1]` only after `fit()` or `load_pretrained()` is called. Calling `output_dim` (or any downstream method dependent on `NATIVE_DIM`) before initialization will raise `RuntimeError`. **L2.2 Slot 1 instantiation order must call `scTOPSubstrate.fit(reference)` OR `scTOPSubstrate.load_pretrained(...)` BEFORE any L7 head module construction.** This binding is enforced for L2.2 specification.
>
> **Finding 4 — §12.2 Anchor Re-Read Trigger Satisfaction (HISTORICAL NOTE):**
>
> §12.2 notes that Souza & Mehta 2026 (the methodological keystone for Decision 1 v2 Commitment 2 scTOP co-equality) was NOT re-read in the L2.1 writing session — primary content was extracted from Q1 synthesis + Decision 1 v2 cited evidence. This was a real anchor re-read trigger gap per Phase B Plan v2 discipline.
>
> **HISTORICAL UPDATE:** The 2026-05-11 corpus-read audit session (the same session that produced this errata) DID re-read Souza & Mehta 2026 in full primary-source form. The anchor re-read trigger for L2.1's scTOP content is now satisfied retroactively. Future modifications to L2.1's scTOP sections (§3.1 through §3.6) must continue to honor the re-read trigger before edits. No content changes to §3 are made under this errata — the existing content is verified consistent with Souza & Mehta's actual claims (Tabula Sapiens 2.0 F1=0.899; tied with TranscriptFormer 0.910/0.907; CPU-runnable vs 1000 H100 GPU; cross-species across all 8 mammals including platypus; disease-state classification).
>
> — Errata applied 2026-05-11 per Charter v1.2 lock + Drift Findings 4, 5, 6 cleanup pass.
