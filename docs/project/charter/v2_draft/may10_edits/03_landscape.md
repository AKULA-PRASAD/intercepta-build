...
**Project's literature survey of 2024-2025 SOTA (added Charter v2.1, May 2026).** The architectural decision to build INTERCEPTA on foundation-model-based cell representations (Innate Response Layer, Chapter 6) is grounded in a concrete literature survey conducted by the project in May 2026, documented in `docs/research/INTERCEPTA_Architectural_Debt_Erratum_2026-05-09.md` Section 5. The survey's key findings:

- **scDrugMap 2025** (*Nature Communications*): foundation model benchmark on 495,000 cells across 60 datasets. scFoundation F1=0.971 (pooled), UCE F1=0.774 (cross-data fine-tuned), scGPT F1=0.858 (zero-shot).
- **scRank 2024** (*Cell Reports Medicine*): GRN-based drug-responsive cell type identification, 71.3% accuracy.
- **CancerFoundation 2024** (bioRxiv): cancer-specific scFM addressing the healthy-cell bias of generic foundation models.
- **DrugFormer, DREEP, DELFOS, scDR**: 2022-2024 publications addressing INTERCEPTA's exact problem space.

For comparison, INTERCEPTA's KAALCURA framework achieves mean AUROC=0.671 across 286 GDSC drugs (canonical implementation, residualized + tissue PCA, validated 2026-05). The metrics are not directly comparable (AUROC vs F1, GDSC vs scDrugMap datasets), but the rough magnitude difference is real: foundation-model-based methods now substantially exceed signature-scoring approaches like KAALCURA on cross-cohort drug response prediction. This honest assessment motivates Charter v2.0's architectural framing: foundation models become the cell representation backbone (Innate Response Layer, Chapter 6; Milestone M1 MC-FMA, Chapter 13), with KAALCURA preserved as a complementary mechanistic-interpretability layer per `docs/INTERCEPTA_Vision_Module1_Amendment.md` (May 6, 2026). The literature survey is itself Layer 1 work per Fullest Vision Research Charter v1.0's research cadence.
