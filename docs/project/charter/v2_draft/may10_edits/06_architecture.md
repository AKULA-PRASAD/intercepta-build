...
## How the Biological Immune System Works
...
## Translation to Computational Architecture
...
## Innate Response Layer — Foundation Models
...
## Adaptive Response Layer — Per-Disease Learning
...
**KAALCURA validation evidence (added Charter v2.1, May 2026).** The mean AUROC=0.671 figure (rounds to 0.6715) is the result of canonical KAALCURA validation on real GDSC data, produced by `code/intercepta_kaalcura_v1.py` (1,046 lines — residualized Z-score + tissue PCA decomposition) operating on the GDSC drug response dataset. Outputs at `results/kaalcura_real_validation_RERUN.csv`. PARPi mechanistic correctness verified per-drug: Olaparib AUROC=0.762, coef_ddr=−1.300; Veliparib AUROC=0.753, coef_ddr=−0.944; Niraparib AUROC=0.750, coef_ddr=−1.565; plus 2 additional PARP inhibitors at AUROC≥0.75. Also: Vorinostat (HDAC inhibitor) AUROC=0.770. Across 286 drugs: 99% above AUROC 0.55 threshold; 91% above 0.60; 70% above 0.65. Documented in `docs/research/INTERCEPTA_Architectural_Debt_Erratum_2026-05-09.md` §3.1-3.2 (this also closes FIX-003 in MASTER_FIXES — the "KAALCURA never validated on real GDSC" issue). KAALCURA's role was further refined per `docs/INTERCEPTA_Vision_Module1_Amendment.md` (May 6, 2026) based on Round 2 BeatAML evidence: KAALCURA is a feature framework with cross-dataset transfer properties (Round 2.2b Q_D PASS at ρ=−0.271, p=0.00125), not a within-dataset standalone predictor (5 FAILs across 3 sub-rounds × 4 method variants on BeatAML within-dataset prediction).

## Memory and Scaling Intelligence
...
## Self-Tolerance and Uncertainty
...
## Failure Modes by Architectural Component
