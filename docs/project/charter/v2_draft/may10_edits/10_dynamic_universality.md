...
## Why Static Architectures Fail at Universality
...
## Dynamic Mechanism Representation
...
**KAALCURA's validated cancer-context role (added Charter v2.1, May 2026).** Within cancer, where KAALCURA was developed, the framework's role was further refined per Round 2 measurement evidence, documented in `docs/INTERCEPTA_Vision_Module1_Amendment.md` (May 6, 2026, tag `vision-module1-amended`). The amendment specifies what KAALCURA does and does not do, based on data:

*What KAALCURA does (validated):*
- Provides interpretable phenotype-state coordinates per cell or sample
- Transfers semantically across datasets (Round 2.2b Q_D PASS: ρ=−0.271, p=0.00125, n=139 drugs, BeatAML→Van Galen scRNA-seq cross-dataset transfer)
...
*What KAALCURA does NOT do:*
- Predict within-dataset drug sensitivity at AUROC ≥0.55 as a standalone 3-axis feature (5 FAILs across 3 sub-rounds × 4 method variants on BeatAML AML drug response prediction)
- Provide marginal contribution above raw RNA features in multi-modal predictors (KAALCURA gain share 0.3% in Round 2.2c LightGBM with 1,034 features; ablation delta −0.0004)

KAALCURA is therefore a feature framework with cross-dataset transfer properties, not a within-dataset standalone predictor. Drug sensitivity prediction is the responsibility of downstream multi-modal predictors that integrate KAALCURA with raw RNA, mutation, pathway, and drug-target features. This is the cancer-context refinement that the dynamic-universality framework above generalizes from: each disease will have its own analog set of mechanism axes with their own validated and invalidated use cases. The lesson is the discipline of distinguishing what mechanism axes can and cannot do empirically, not just what they are designed to do.

## Dynamic Phenotype Targets
...
## How Universality Is Earned, Not Claimed
