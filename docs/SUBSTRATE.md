...
pathogens), a **hard host-non-homology safety filter**, a **resistance-robustness** flag (SYNLETH1: monotherapy-robust
vs isozyme-buffered *combination-required* targets, from metabolic bypass), and a **condition-robustness** flag (CONDROB1:
targets essential across nutrient environments incl. a host-like medium = environment-independent — a validated quality axis,
79% vs 48% experimental-essential). Confidence is calibrated to accuracy (CALIB1).
