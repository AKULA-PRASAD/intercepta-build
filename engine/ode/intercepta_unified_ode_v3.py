#!/usr/bin/env python3
"""
INTERCEPTA Unified Tumor Dynamics ODE — v3
============================================
4 states x 20 bins = 80 compartments.

Extends intercepta_unified_ode.py (v2) with:
...
class UnifiedODEv3:
    ...
    def __init__(self, n_bins: int = N_BINS_DEFAULT,
        ...
        self.g_base = R_MAX * (1.0 - ALPHA_R * self.x)  # per-bin intrinsic growth
        self.g_mods = np.array([G_MOD_S, G_MOD_M, G_MOD_V, G_MOD_N])
        self.brca_frac = brca_profile(brca_profile_name, n_bins)  # per bin
    ...
    def set_drugs(self, drug_names: List[str], duration_days: int):
        ...
        for name in drug_names:
            ...
            self.active_drugs.append((name, self._drug_library[name]))
    ...
    def deriv(self, t: float, y: np.ndarray) -> np.ndarray:
        ...
        y = np.maximum(y, 0.0)
        N_total = y.sum()
        logistic = max(1.0 - N_total / K_CAP, 0.0)
        ...
        mu_s_to_n = MU_S_TO_N_TX if ar_therapy_active else MU_S_TO_N_BASE
        ...
        for s_idx in range(self.N_STATES):
            ...
            if N >= 2:
                # Interior
                diff[1:-1] = BETA * (ns[:-2] - 2*ns[1:-1] + ns[2:]) / (self.dx**2)
                # Boundaries (reflective)
                diff[0] = BETA * (ns[1] - ns[0]) / (self.dx**2)
                diff[-1] = BETA * (ns[-2] - ns[-1]) / (self.dx**2)
        ...
        if ar_therapy_active:
            ...
            loss_from_S = (MU_S_TO_M + MU_S_TO_V + mu_s_to_n) * ns_S
            dy[self._state_slice(0)] -= loss_from_S
            dy[self._state_slice(1)] += MU_S_TO_M * ns_S
            dy[self._state_slice(2)] += MU_S_TO_V * ns_S
...
def simulate_cohort(drug_list: List[str],
    ...
    for i in range(n_patients):
        # Patient-level perturbations (log-normal, CV~15% per parameter)
        # Only perturb parameters that matter for inter-patient variability;
        # keep structural parameters fixed.
        burden_factor = float(np.exp(rng.normal(0, 0.25)))  # CV ~25% burden
        N0_i = 0.15 * burden_factor
        N0_i = min(N0_i, 0.8)  # cap to avoid starting above carrying capacity

        model = UnifiedODEv3(brca_profile_name=brca_profile_name)
        model.set_drugs(drug_list, duration_days)

        y0 = build_initial_state(velocity_csv, model.N, N0_i, state_fracs)
...
def run_validation(n_patients: int = COHORT_N_PER_ARM,
                   save_path: str = '../results/unified_v3_validation.json'):
    ...
    for trial_name, spec in trials.items():
        ...
        result = {
            ...
            'cox_hr': hr_res['hr'],
            'cox_hr_ci': [hr_res['hr_ci_lower'], hr_res['hr_ci_upper']],
        ...
        if spec.get('target_hr') is not None:
            ...
            passed = (lo <= hr_res['hr'] <= hi)
            result['pass_criterion'] = f"HR in [{lo}, {hi}]"
        ...
        print(f"  Cox HR: {hr_res['hr']:.3f} "
              f"[95% CI {hr_res['hr_ci_lower']:.3f}–{hr_res['hr_ci_upper']:.3f}]  "
    ...
    out = {
        'model': 'INTERCEPTA Unified ODE v3',
        'compartments': 80,
...
if __name__ == '__main__':
    ...
    ap.add_argument('--out', type=str, default='../results/unified_v3_validation.json')
    args = ap.parse_args()
