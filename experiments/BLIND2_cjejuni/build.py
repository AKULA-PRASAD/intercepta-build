"""BLIND2 Stage 1 (LOCK) — produce and freeze the FBA-essentiality predictions for C. jejuni NCTC 11168 from the de-novo
CarveMe GEM ALONE. NO experimental essentiality is read here (blindness). Output: results/LOCKED_predictions.tsv (+ .sha256).
Env: metabolic (cobra + GLPK). Identical protocol to BLIND1/build.py (only gid->accession parsing adapts to CarveMe gene ids).
"""
...
GEM = os.path.join(DATA, "blind2", "cjejuni.xml"); FASTA = os.path.join(DATA, "blind2", "cjejuni.fasta")


def gid2acc(g):
    """CarveMe gene id like 'tr_Q0P9W1_Q0P9W1_CAMJE' or 'sp_Q9PIK1_PANB_CAMJE' -> UniProt accession (2nd token)."""
...
def main():
    ...
    for r in sg.itertuples():
        ...
        # round KO growth to 6dp for GLPK jitter reproducibility, then normalize to WT ratio at 4dp (as BLIND1)
        rows.append((acc, acc2sym.get(acc, ""), ess, round(round(float(r.growth), 6) / wt, 4)))
    rows.sort()
