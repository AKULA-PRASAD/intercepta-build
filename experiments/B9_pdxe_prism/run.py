"""B9 — PRISM-POWERED external validation on PDXE. Implements prereg/B9_pdxe_prism_powered.md.
...
print("B9 PDXE PRISM-powered external validation | sklearn", sklearn.__version__, flush=True)
...
out = {"git_sha": os.popen("git rev-parse HEAD").read().strip(), "python": sys.version.split()[0],
       ...
       "seed": SEED, "K": K, "cohort": "PDXE (public PDX); PRISM-trained transfer", "n_drugs": n, "drugs": usable,
...
json.dump(out, open(os.path.join(HERE, "results", "B9_metrics.json"), "w"), indent=2)
print("wrote results/B9_metrics.json")
