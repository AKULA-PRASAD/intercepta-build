#!/bin/bash
# Robust per-chunk Boltz-2 runner for the AFFINITY1 array. Restartable + NFS-safe.
# Args: $1 = chunk YAML dir, $2 = chunk out dir. Requires env: BENV, BOLTZ_CACHE.
# Success criterion = every input YAML has an affinity_*.json (NOT the boltz exit code):
#   boltz can throw a benign NFS ".nfsXXXX Errno 16 device busy" during tmp cleanup AFTER a
#   successful prediction — we must not let that mark the task failed.
