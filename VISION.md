...
(TID3: bacteria→parasite→fungus recovery falls to zero, and abstention does not flag the failure) — and no label-free
signal predicts which organisms it fails on (TID4), so the system cannot cheaply know when it is out of its depth. **However, a mechanistic (non-homology) signal — FBA gene-essentiality — DOES break the conservation ceiling for metabolic targets (MET1: CV ΔAUROC +0.132, odds ratio 8.6 on E. coli); it REPLICATES in the 2 bacteria with enough drug targets to test reliably (MET2: E. coli +0.053, M. tuberculosis +0.040 — broad bacterial generalization remains data-limited by drug-target sparsity, not signal failure); and it translates from a coefficient into a better RANKED target shortlist — clearly in E. coli (MET3: precision@k 0.28→0.35, 4.9→6.2× enrichment), weakly in Mtb (+0.015 ≈ one target) — pointing to MECHANISM, not more homology, as the path forward. But this mechanistic win is scoped to METABOLIC targets: the natural extension to the NON-metabolic (FBA-blind) half via PPI-network centrality FAILED the confound test (MET4) — its apparent +0.128 lift is a STUDY-BIAS artifact (drug targets are the most-studied proteins → more PPI edges; study-intensity alone predicts targethood at AUROC 0.826, and the lift collapses to −0.004 once controlled) — so the FBA-blind half remains genuinely hard, and the honest claim stays: mechanism breaks the ceiling for metabolic targets, not (yet) for the rest.** **Binding** carries a real-but-weak zero-data signal (C1; the GNINA rescoring lever is Linux/CUDA-only,
